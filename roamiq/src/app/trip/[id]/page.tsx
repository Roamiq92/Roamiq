    ...
    <div className="min-h-screen bg-[#0a0b0f] text-white flex flex-col items-center">
      ...
      {coords && (
        <div className="w-full max-w-4xl h-72 rounded-2xl overflow-hidden mb-10">
          <Map center={coords} zoom={13} style={{ width: "100%", height: "100%" }}>
            <Tile
        ...
        {data.hotels && (
          <DashboardSection title="🏨 Hotel">
            {data.hotels.map((h: any, i: number) => (
              <BookingCard key={i}
                title={h.name}
                subtitle={`${h.rating}★ €${h.price_per_night}/notte`}
                link={h.link}
              />
            ))}
          </DashboardSection>
      ...
      <footer className="py-10 text-center">
        <p className="text-gray-400 mb-2">
          Costo stimato <span className="text-orange-400 font-bold">€{data.estimated_cost}</span>
        </p>
...
function BookingCard({ title, subtitle, link }: { title: string; subtitle: string; link: string }) {
  return (
    <a
      href={link}
      target="_blank"
      rel="noreferrer"
      className="group p-4 border border-white/10 bg-white/5 hover:bg-white/10 rounded-xl transition block"
    >
